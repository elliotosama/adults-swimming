...
function captainHasMojibakeMarker(string $value): bool
{
    return preg_match('/[╪┘┌├┬]/u', $value) === 1;
}

function captainHasArabic(string $value): bool
{
    return preg_match('/[\x{0600}-\x{06FF}]/u', $value) === 1;
}

function repairCaptainMojibake(string $value): string
{
    if (!captainHasMojibakeMarker($value) || captainHasArabic($value)) {
        return $value;
    }

    $current = $value;
    for ($i = 0; $i < 3; $i++) {
        if (!captainHasMojibakeMarker($current)) {
            break;
        }
...
foreach ($select as $row) {
    $old = (string) $row['captain_name'];
    $new = repairCaptainMojibake($old);
    if ($new === $old) {
        continue;
    }
